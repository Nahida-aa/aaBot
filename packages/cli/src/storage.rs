...
fn data_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("XDG_DATA_HOME") {
        ...
        PathBuf::from(home).join(".local").join("share").join("aa").join("sessions")
    } else {
...
pub(crate) fn load_session(session_id: &str) -> anyhow::Result<Vec<Message>> {
    let path = session_path(session_id);
    let file: SessionFile = serde_json::from_reader(std::fs::File::open(&path)?)?;
    Ok(file.messages)
}

pub(crate) fn load_session_file(session_id: &str) -> anyhow::Result<SessionFile> {
    let path = session_path(session_id);
    Ok(serde_json::from_reader(std::fs::File::open(&path)?)?)
}

pub(crate) fn list_sessions() -> anyhow::Result<Vec<SessionFile>> {
    let dir = data_dir();
    if !dir.exists() {
        return Ok(vec![]);
    }
